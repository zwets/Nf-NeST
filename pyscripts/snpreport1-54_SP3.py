...
# process=subprocess.Popen(['samtools', 'index', args.name+ "_SR.bam"], stdout=subprocess.PIPE)
...
coverageinfo=open(args.coveragefile,'r')
#print(coverageinfo)
#reader2=csv.reader(coverageinfo.splitlines(),delimiter='\t', skipinitialspace=True)
#print(reader2)

#tempcov1=[]
tempcovwordlist1=[]
for line in coverageinfo:
    line=line.rstrip()
    count=0
    tempcovword1=""
...
for x in range(len(Genelist2)):
    ...
    #print(AAPoslist12[x])
            ...
            if testdicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]==dicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]:
                #print("False")
                filteredmutationlist1+=["Wildtype"+"_"+"N/A"]
            if DP4list1[x]!="None":
                if float(DP4list1[x])>=0.5:
                    if testdicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]!=dicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]:
                        filteredmutationlist1+=["Major"+"_"+str(DP4list1[x])]
                if float(DP4list1[x])<0.5:
                    if testdicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]!=dicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]:
                        filteredmutationlist1+=["Minor"+"_"+str(DP4list1[x])]
            if DP4list1[x]=="None":
                #print('TRUE')
                if testdicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]!=dicttransAA1[Genelist2[x]][BASEAAPOSdic1[Genelist2[x],(POSlist2[x])]-1]:
                    filteredmutationlist1+=["Wildtype"+"_"+"N/A"]
...
for y in wholepotlist1:
    ...
    for x in range(len(combinedGenePOSlist1)):
        ...
        if x==len(combinedGenePOSlist1)-1 and ([y[0],int(y[1])] not in combinedGenePOSlist1) and ([y[0],int(y[1])] not in combinedwildgeneAA) and (y[0] in tempcovwordlist1):
            ...
            filteredAFlist12+=["NA"]
            filteredmutationlist12+=["Wildtype_N/A"]
            filteredQDlist12+=["NA"]
